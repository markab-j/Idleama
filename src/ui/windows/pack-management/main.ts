...
document.addEventListener("DOMContentLoaded", () => {
  ...
  once(PackManagementEvent.INIT_WINDOW, async (event: PackInitEvent) =>
    renderPacks(event.payload.packs),
  );
...
function renderPacks(packs: CharacterPackState[]): void {
  const container = document.getElementById(
    "packs-container",
  ) as HTMLDivElement;
  container.innerHTML = "";

  packs.forEach((packState) => {
    const packElement = createPackElement(packState);
    container.appendChild(packElement);
  });
}

function createPackElement(pack: CharacterPackState): HTMLElement {
  ...
  packItem.innerHTML = `
      <div class="checkbox-container">
        <input type="checkbox" id="${elementId}" class="pack-checkbox" ${pack.enabled ? "checked" : ""}>
